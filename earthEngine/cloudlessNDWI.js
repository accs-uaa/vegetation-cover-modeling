/* -*- coding: utf-8 -*-
---------------------------------------------------------------------------
Cloud-reduced Best Pixel Composite NDWI
Author: Timm Nawrocki, Alaska Center for Conservation Science
Created on: 2018-05-30
Usage: Must be executed from the Google Earth Engine code editor.
Description: This script produces a cloud-reduced best pixel composite Normalized Difference Water Index (NDWI) using Top-Of-Atmosphere (TOA) reflectance. See Chander et al. 2009 for a description of the TOA reflectance method. NDWI was calculated as (Green - NIR)/(Green + NIR), where Green is Landsat 8 Band 3 and NIR (near infrared) is Landsat 8 Band 5, using the Google Earth Engine normalized difference algorithm.
---------------------------------------------------------------------------*/
...
// Define a function for NDWI calculation with cloud-reduction mask.
var ndwiCloudless = function(image) {
  ...
  return image.addBands(ndwi).updateMask(cloudMask);
};
...
var ndwiCollection = landsatFiltered.map(ndwiCloudless);

// Make a best pixel composite from the image collection.
var compositeBest = ndwiCollection.qualityMosaic('NDWI');
print('Best Pixel NDWI:', compositeBest)

// Define parameters for NDWI.
var ndwiParams = {
  bands: ['NDWI'],
  min: -1,
  max: 1,
...
Map.addLayer(compositeBest, ndwiParams, 'NDWI Composite');

// Create a single band image with NDWI.
var ndwiBest = ee.Image(compositeBest).select(['NDWI']);

// Export image to Google Drive.
Export.image.toDrive({
  image: ndwiBest,
  description: 'cloudlessLandsat8NDWI',
  scale: 30,
