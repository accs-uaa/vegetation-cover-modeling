/* -*- coding: utf-8 -*-
---------------------------------------------------------------------------
Cloud-reduced Best Pixel Composite EVI-2
Author: Timm Nawrocki, Alaska Center for Conservation Science
Created on: 2018-05-30
Usage: Must be executed from the Google Earth Engine code editor.
Description: This script produces a cloud-reduced best pixel composite Enhanced Vegetation Index-2 (EVI2) using Top-Of-Atmosphere (TOA) reflectance. See Chander et al. 2009 for a description of the TOA reflectance method. EVI-2 was calculated as (Red - Green) / (Red + [2.4 x Green] + 1), where Red is Landsat 8 Band 4 and Green is Landsat 8 Band 3, using the Google Earth Engine normalized difference algorithm.
---------------------------------------------------------------------------*/
...
// Define a function for EVI2 calculation with cloud-reduction mask.
var evi2Cloudless = function(image) {
  ...
  var evi2 = red.subtract(green).divide(red.add(green.multiply(2.4)).add(1)).rename('EVI2');
  // Return the masked image with an EVI-2 band.
  return image.addBands(evi2).updateMask(cloudMask);
};
...
// Calculate EVI2 for image collection and add as new band.
var evi2Collection = landsatFiltered.map(evi2Cloudless);

// Make a best pixel composite from the image collection.
var compositeBest = evi2Collection.qualityMosaic('EVI2');
print('Best Pixel EVI2:', compositeBest)

// Define parameters for EVI2.
var evi2Params = {
  bands: ['EVI2'],
  min: -1,
  max: 1,
...
Map.addLayer(compositeBest, evi2Params, 'EVI2 Composite');

// Create a single band image with EVI2.
var evi2Best = ee.Image(compositeBest).select(['EVI2']);

// Export image to Google Drive.
Export.image.toDrive({
  image: evi2Best,
  description: 'cloudlessLandsat8EVI2',
  scale: 30,
