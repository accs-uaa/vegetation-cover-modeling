/* -*- coding: utf-8 -*-
---------------------------------------------------------------------------
Cloud-reduced Greenest Pixel Composite NDVI
Author: Timm Nawrocki, Alaska Center for Conservation Science
Created on: 2018-05-30
Usage: Must be executed from the Google Earth Engine code editor.
Description: This script produces a cloud-reduced greenest pixel composite Normalized Difference Vegetation Index (NDVI) using Top-Of-Atmosphere (TOA) reflectance. See Chander et al. 2009 for a description of the TOA reflectance method. NDVI was calculated as (NIR - Red)/(NIR + Red), where NIR (near infrared) is Landsat 8 Band 5 and Red is Landsat 8 Band 4, using the Google Earth Engine normalized difference algorithm.
---------------------------------------------------------------------------*/
...
// Define a function for NDVI calculation with cloud-reduction mask.
var ndviCloudless = function(image) {
  ...
  return image.addBands(ndvi).updateMask(cloudMask);
};
...
var ndviCollection = landsatFiltered.map(ndviCloudless);
...
// Create a single band image with NDVI.
var ndviGreenest = ee.Image(compositeGreenest).select(['NDVI']);

// Export image to Google Drive.
Export.image.toDrive({
  image: ndviGreenest,
  description: 'cloudlessLandsat8NDVI',
  scale: 30,
