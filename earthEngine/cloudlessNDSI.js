/* -*- coding: utf-8 -*-
---------------------------------------------------------------------------
Cloud-reduced Best Pixel Composite NDSI
Author: Timm Nawrocki, Alaska Center for Conservation Science
Created on: 2018-05-30
Usage: Must be executed from the Google Earth Engine code editor.
Description: This script produces a cloud-reduced best pixel composite Normalized Difference Snow Index (NDSI) using Top-Of-Atmosphere (TOA) reflectance. See Chander et al. 2009 for a description of the TOA reflectance method. NDSI was calculated as (Green - SWIR1) / (Green + SWIR1), where Green is Landsat 8 Band 3 and SWIR1 (short-wave infrared 1) is Landsat 8 Band 6, using the Google Earth Engine normalized difference algorithm.
---------------------------------------------------------------------------*/
...
// Define a function for NDSI calculation with cloud-reduction mask.
var ndsiCloudless = function(image) {
  ...
  return image.addBands(ndsi).updateMask(cloudMask);
};
...
// Calculate NDSI for image collection and add as new band.
var ndsiCollection = landsatFiltered.map(ndsiCloudless);

// Make a best pixel composite from the image collection.
var compositeBest = ndsiCollection.qualityMosaic('NDSI');
print('Best Pixel NDSI:', compositeBest)

// Define parameters for NDSI.
var ndsiParams = {
  bands: ['NDSI'],
  min: -1,
  max: 1,
...
Map.addLayer(compositeBest, ndsiParams, 'NDSI Composite');

// Create a single band image with NDSI.
var ndsiBest = ee.Image(compositeBest).select(['NDSI']);

// Export image to Google Drive.
Export.image.toDrive({
  image: ndsiBest,
  description: 'cloudlessLandsat8NDSI',
  scale: 30,
