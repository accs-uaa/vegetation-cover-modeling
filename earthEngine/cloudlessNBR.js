/* -*- coding: utf-8 -*-
---------------------------------------------------------------------------
Cloud-reduced Best Pixel Composite NBR
Author: Timm Nawrocki, Alaska Center for Conservation Science
Created on: 2018-05-30
Usage: Must be executed from the Google Earth Engine code editor.
Description: This script produces a cloud-reduced best pixel composite Normalized Burn Ratio (NBR) using Top-Of-Atmosphere (TOA) reflectance. See Chander et al. 2009 for a description of the TOA reflectance method. NBR was calculated as (NIR - SWIR2) / (NIR + SWIR2), where NIR (near infrared) is Landsat 8 Band 5 and SWIR2 (short-wave infrared 2) is Landsat 8 Band 7, using the Google Earth Engine normalized difference algorithm.
---------------------------------------------------------------------------*/
...
var nbrCloudless = function(image) {
  ...
  return image.addBands(nbr).updateMask(cloudMask);
};
...
// Calculate NBR for image collection and add as new band.
var nbrCollection = landsatFiltered.map(nbrCloudless);

// Make a best pixel composite from the image collection.
var compositeBest = nbrCollection.qualityMosaic('NBR');
print('Best Pixel NBR:', compositeBest)

// Define parameters for NBR.
var nbrParams = {
  bands: ['NBR'],
  min: -1,
  max: 1,
...
Map.addLayer(compositeBest, nbrParams, 'NBR Composite');

// Create a single band image with NBR.
var nbrBest = ee.Image(compositeBest).select(['NBR']);

// Export image to Google Drive.
Export.image.toDrive({
  image: nbrBest,
  description: 'cloudlessLandsat8NBR',
  scale: 30,
